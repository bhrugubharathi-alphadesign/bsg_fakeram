...
enum SRAMcell_type // Alireza
{
    std_6T, // standard 6T SRAM cell
    std_8T  // standard 8T SRAM cell
};

typedef struct{ // Alireza2
    int Nfin; // number of fins
    double Lphy; // gate length
    double Ioff; // off current
} SRAMTransistorParams;

class SRAMCellParameters // Alireza2
{
  private:
    bool PleakAT_given, PleakCC_given;
    double width, height;
    SRAMcell_type type; // standard 6T, or 8T SRAM cell
    bool dg_control; // dual-gate controlled SRAM cell?
    double Pleak_acctx; // leakage power of access transistors (bitline leakage power)
    double Pleak_ccinv; // leakage power of cross-coupled inverters
    // access (acc), pull up (pup), and pull down (pdn) transistors -->
    SRAMTransistorParams acc, pup, pdn;
    // isolator transistor (M7), and read access transistor (M8) in 8T SRAM
	 SRAMTransistorParams iso, rac;
  
  public:
    SRAMCellParameters():
      type(std_6T), dg_control(false), Pleak_acctx(0.0), Pleak_ccinv(0.0),
      PleakAT_given(false), PleakCC_given(false), width(0.0), height(0.0) { }
    
    void setType(SRAMcell_type t) { type = t; }
    SRAMcell_type getType() { return type; }
    ...
    double calc_width(double lambda_um, double p_fin, double t_si);
	 void print_transistor_params();
};
...
class InputParameter
{
  public:
    ...
    char data_array_peri_tech_file[1000];
    char tag_array_cell_tech_file[1000];
