...
void Mat::compute_power_energy()
{
  ...
  power_row_decoders.writeOp.dynamic       = row_dec->power.writeOp.dynamic; // Alireza: for 8T SRAM cell

  if (is_fa == false)
  {
    power_bl_precharge_eq_drv.readOp.dynamic *= num_subarrays_per_mat;
    power_row_decoders.readOp.dynamic        *= num_subarrays_per_mat;
    power_row_decoders.writeOp.dynamic       *= num_subarrays_per_mat; // Alireza: for 8T SRAM cell
  }
  else if (dp.is_tag == true)
  {
    power_row_decoders.readOp.dynamic         = power_fa_decoder.readOp.dynamic;
    power_row_decoders.writeOp.dynamic        = power_fa_decoder.readOp.dynamic; // Alireza: for 8T SRAM cell
  }
